...
from .config import *
from .predict import predict_use_two_algo
...
def predict():
    ...
    try:
        ...
        predictions = predict_use_two_algo(data)
        predictions.to_csv(os.path.join(UPLOAD_FOLDER, 'output.csv'), index=False)

        answer['Сообщение'] = 'Все прошло супер!'
        answer['Успех'] = True
        answer['Путь'] = 'output'
        return answer

    except Exception as _ex:
        answer['Сообщение'] = 'Произошла ошибка! Подробнее смотри в "Ошибка"'
        answer['Ошибка'] = str(_ex)
        answer['Успех'] = False
        return answer
